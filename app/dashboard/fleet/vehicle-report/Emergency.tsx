...
function Emergency() {
  const router = useRouter();
      ...
      const data: Row[] =
      Array.isArray(vehicleReport?.data) // Check if it's an array
        ? vehicleReport.data
            .filter((report: any) => report.reportType === "EMERGENCY") // Filter for active vehicles
     ...
     const columns: Column[] = [
      {
        title: "S/N",
        field: "index",
        render: (rowData) => <div className="whitespace-nowrap">{rowData.index}</div>,
      },
       {
         title: "Description",
         field: "description",
  ...
  
       {
         ...
         render: (rowData) => {
           ...
           return (
             <div onClick={() => handleVehicleReportDetails(rowData?.id)} className=" cursor-pointer relative">
              View
             </div>
           );
         },
